...
func (m *MockVideoService) GetVideosByCategory(c *gin.Context) ([]types.ResponseVideo, error) {
	args := m.Called(c)
	return args.Get(0).([]types.ResponseVideo), args.Error(1)
}
...
func TestVideoController_GetVideosByCategory(t *testing.T) {
	// Mock VideoService
	mockService := new(MockVideoService)
	...
	// Set up expectations for the mock service
	mockService.On("GetVideosByCategory", mock.Anything).Return(expectedVideos, nil)
	...
	// Create a test context using httptest
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/videos/category?keyword=category1", nil)
	ctx, _ := gin.CreateTestContext(w)
	ctx.Request = req

	// Call the method under test
	videoController.GetVideosByCategory(ctx)
...
func TestVideoController_GetVideosByCategory_ErrorInService(t *testing.T) {
	// Mock VideoService
	mockService := new(MockVideoService)

	// Set up expectations for the mock service to return an error
	mockService.On("GetVideosByCategory", mock.Anything).Return([]types.ResponseVideo{}, errors.New("failed"))
